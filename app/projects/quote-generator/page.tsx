...
import { Download, RefreshCw, LayoutTemplate, Type, Palette, Sparkles, PaintBucket, Maximize } from "lucide-react"
import { cn } from "@/lib/utils"

// Add custom scrollbar styles to the global CSS
...
export default function QuoteCardGenerator() {
  ...
  // New state for size options
  const [selectedSize, setSelectedSize] = useState("medium")
  const [customWidth, setCustomWidth] = useState(600)
  const [customHeight, setCustomHeight] = useState(600)
  const [previewScale, setPreviewScale] = useState(1)

  // Add a new state variable for tracking content overflow
  ...
  // Add a ref for the content area
  ...
  const handleDownload = async () => {
    ...
    if (cardRef.current) {
      try {
        ...
        link.download = `${filePrefix}-${selectedLayout}-${selectedSize}-quote-card.png`
        link.href = dataUrl
        link.click()
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Add a style tag to inject the scrollbar styles */}
      <style jsx global>
        {scrollbarStyles}
      </style>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <Tabs defaultValue="content" className="w-full">
                ...
                <TabsContent value="content" className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">This will appear in the footer of your quote card.</p>
                  </div>
                </TabsContent>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[400px] overflow-y-auto p-2">
                    {colorPalettes.map((palette) => (
                      ...
                      >
                        ...
                        >
                          <span className={cn("text-sm font-bold capitalize", palette.textColor)}>{palette.name}</span>
                        </div>
                        <div className="bg-background py-1 px-2">
                  ...
                  <div className="p-4 border rounded-md bg-muted/30">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="font-mono">{currentWidth}px</p>
                      </div>
                      <div>
                        <span className="text-sm text-muted-foreground">Height:</span>
                        <p className="font-mono">{currentHeight}px</p>
                      </div>
                    </div>
                  </div>
                </TabsContent>
        ...
        <div>
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <h3 className={cn("font-bold", activeTextColor, fontScale.title)}>{cardTitle || "My Quote Card"}</h3>
                ...
                {/* Update the footer to ensure text truncation works properly */}
                <div
                  className={cn(
                    "border-t border-white/20 flex justify-between items-center flex-shrink-0",
                    paddingScale.footer,
                  )}
                >
                  <div className={cn("flex items-center", activeTextColor, fontScale.footer, "max-w-[45%]")}>
                    <span className="truncate">
                      {useCustomPalette ? `Style: ${palette.name}` : `Made by ${selectedTemplate}`}
                    </span>
                  </div>
                  <div className={cn("flex items-center", activeTextColor, fontScale.footer, "max-w-[45%]")}>
                    <Sparkles className="h-3 w-3 sm:h-4 sm:w-4 mr-1 flex-shrink-0" />
                    <span className="truncate">{footerDescription || "Generated with purpose"}</span>
                  </div>
                </div>
              </div>

              <div className="absolute bottom-2 right-2 bg-background/80 backdrop-blur-sm text-xs px-2 py-1 rounded-md border">
          ...
          {contentOverflow && (
            <div className="mt-2 p-2 bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-200 text-sm rounded-md flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              <p>
                Your quote is longer than the card can display. Content will be scrollable in the preview and final
                image.
              </p>
            </div>
          )}
          ...
          <div className="mt-4 p-4 bg-muted rounded-lg">
            <h3 className="font-medium mb-1">{useCustomPalette ? palette.name : template.name}</h3>
            <p className="text-sm text-muted-foreground">{activeDescription}</p>
            <div className="mt-2 pt-2 border-t border-border">
              <p className="text-xs text-muted-foreground">
                Size:{" "}
                <span className="font-medium">{selectedSize === "custom" ? "Custom" : selectedSizeOption.name}</span> •
                Layout: <span className="font-medium">{layout.name}</span>
              </p>
            </div>
          </div>
        </div>
